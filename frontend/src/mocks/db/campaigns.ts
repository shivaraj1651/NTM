...
export function generateCreativeAssets(campaignId: string): CreativeAssets {
  ...
  const images: ImageAsset[] = [
    { id: `${campaignId}-img-1`, format: 'square',    url: 'https://placehold.co/1024x1024/1a1a2e/ffffff?text=Square+Ad',    approved: null, revision_count: 0 },
    { id: `${campaignId}-img-2`, format: 'landscape', url: 'https://placehold.co/1344x768/16213e/ffffff?text=Landscape+Ad', approved: null, revision_count: 0 },
    { id: `${campaignId}-img-3`, format: 'portrait',  url: 'https://placehold.co/768x1344/0f3460/ffffff?text=Portrait+Ad',  approved: null, revision_count: 0 },
  ]

  const audio: AudioAsset[] = [
    { id: `${campaignId}-aud-1`, format: 'radio',        voice_style: 'warm',          url: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3', duration_seconds: 30, approved: null, revision_count: 0 },
    { id: `${campaignId}-aud-2`, format: 'tvc_vo',       voice_style: 'authoritative', url: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3', duration_seconds: 30, approved: null, revision_count: 0 },
    { id: `${campaignId}-aud-3`, format: 'social_video', voice_style: 'youthful',      url: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3', duration_seconds: 30, approved: null, revision_count: 0 },
  ]
...
export function flattenCreativeAssets(
  ...
  assets.copy.forEach((copy, i) => {
    flat.push({
      ...
      content: {
        ...
        preview: typeof copy.variants[0]?.content === 'string' ? copy.variants[0].content.slice(0, 120) : '',
      },
      validation_status: 'ai_draft',
      created_at: now,
      updated_at: null,
  ...
  assets.scripts.forEach((script) => {
    flat.push({
      ...
      platform: script.format ?? 'script',
      content: {
        format: script.format ?? 'script',
        label: (script.format ?? 'script').replace(/_/g, ' '),
