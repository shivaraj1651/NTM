...
def _make_engine():
    ...
    Celery runs each task in its own `asyncio.run()` loop. A module-level/cached
    engine binds its asyncpg connections to the first loop and then fails on later
    tasks ("another operation is in progress"). So we build a new engine per task
    invocation with NullPool (no cross-call connection reuse) and dispose it after.
    """
